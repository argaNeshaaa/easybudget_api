...
export const insertTransactionsModels = async (walletId, categoryId, accountId, type, amount, description, date) => {
    const query = `INSERT INTO transactions (wallet_id, category_id, account_id, type, amount, description, date, created_at) 
    VALUES (?, ?, ?, ?, ?, ?, ? ,NOW())
    `;
    const [result] = await db.promise().query(query, [walletId, categoryId, accountId, type, amount, description, date]);
