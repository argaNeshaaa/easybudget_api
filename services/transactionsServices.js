...
export const getWeeklyTransactionsListServices = async (userId) => {
  try {
    const result = await getWeeklyTransactionsListModels(userId);
    return result;
  } catch (error) {
    throw ApiError.database("Failed to fetch weekly transactions list");
  }
};

export const getMonthlySummaryServices = async (userId) => {
  try {
    const result = await getMonthlySummaryModels(userId);
    return result;
  } catch (error) {
    throw ApiError.database("Failed to fetch monthly summary");
  }
};

export const getAccountMonthlyStatsServices = async (accountId) => {
  try {
    const result = await getAccountMonthlyStatsModels(accountId);
    return result;
  } catch (error) {
    throw ApiError.database("Failed to fetch account statistics");
  }
};
