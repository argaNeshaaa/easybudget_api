import { deleteAccountsModels, findAccountsByIdModels, findAccountsByWalletIdModels, findAllAccountsModels, insertAccountsModels, updateAccountsModels } from "../models/accountsModels.js";
...
export const insertAccountsServices = async (walletId, accountName, accountNumber, balance, accountType) => {
    try {
        const wallet = await findWalletsByIdServices(walletId);
        if (!wallet) throw ApiError.notFound("Wallets");
        const result = await insertAccountsModels(walletId, accountName, accountNumber, balance, accountType);
...
export const updateAccountsServices = async (id, fields) => {
    try {
        ...
        const forbiddenFields = ["account_id", "wallet_id", "created_at", "updated_at"];
