import { deleteBusinessesServices, findAllBusinessesServices, findBusinessesByIdServices, findBusinessesByUserIdServices, insertBusinessesServices, updateBusinessesServices } from "../services/businessesServices.js"
...
export const insertBusinessesControllers = async (req, res, next) => {
    try {
        ...
        const result = await insertBusinessesServices(user_id, name, industry, address);

        createdResponse(res, { id: result.insertId}, "Business Created Succesfully");
    } catch (error) {
        next(error);
    }
};
