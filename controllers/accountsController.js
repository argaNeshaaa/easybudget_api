...
import { deleteAccountsServices, findAccountsByIdServices, findAccountsByWalletIdServices, findAllAccountsServices, insertAccountsServices, updateAccountsServices } from "../services/accountsServices.js";
...
export const insertAccountsControllers = async (req, res, next) => {
    try {
        const { wallet_id, account_name, account_number, balance, account_type } = req.body;
        const result = await insertAccountsServices(wallet_id, account_name, account_number, balance, account_type);

        createdResponse(res, {id: result.insertId}, "Account created Succesfully");
    } catch (error) {
        next(error);
    }
};
