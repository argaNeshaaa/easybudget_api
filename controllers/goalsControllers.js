import { deleteGoalsServices, findAllGoalsServices, findGoalsByIdServices, findGoalsByUserIdServices, insertGoalsServices, updateGoalsServices } from "../services/goalsServices.js"
...
export const insertGoalsControllers = async (req, res, next) => {
    try {
        const {user_id, name, target_amount, current_amount, deadline, status} = req.body;
        const result = await insertGoalsServices(user_id, name, target_amount, current_amount, deadline, status);
